...
export default function ThirdFeatures() {
    return (
        <div style={{backgroundColor:'#F4F5FC'}} className='w-full relative h-full flex flex-col lg:flex-row justify-center items-center px-6 py-20 my-4 lg:px-20' >
            ...
            <div className='w-full flex flex-row z-20 relative justify-end' >
                <button onClick={()=> Router.push('/register')} style={{backgroundColor:'#1526A7'}} className=' lg:w-40 w-full h-10 lg:mx-2  font-Inter-SemiBold text-sm rounded-sm px-4 text-white' >Create Account</button>
                <button onClick={()=> Router.push('/login')} style={{borderColor:'#1526A7', border:'1px solid' }} className=' lg:w-40 w-full h-10 lg:mx-2 ml-4 lg:ml-0 font-Inter-SemiBold text-sm rounded-sm px-4' >Get Started</button>
            </div>
        </div>
    )
}
